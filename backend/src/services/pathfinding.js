...
// Discretize bounds for Karnataka (Bengaluru & Mysuru region)
...
export function findOptimizedRoute(startLoc, endLoc, activeDisasters, targetDisasterId) {
  ...
  // 2. Mark obstacles on the grid.
  // A cell is an obstacle if its center coordinate falls inside any disaster radius,
  // EXCEPT the disaster we are targeting.
  for (let y = 0; y < GRID_ROWS; y++) {
    for (let x = 0; x < GRID_COLS; x++) {
      ...
      for (const disaster of activeDisasters) {
        // Skip the target disaster itself so we can land there!
        if (disaster._id.toString() === targetDisasterId?.toString()) continue;

        const distance = getDistance(cellLat, cellLng, disaster.location.lat, disaster.location.lng);
        if (distance <= disaster.radius) {
          grid[y][x] = 1; // Mark as blocked (danger zone)
          break;
        }
      }
    }
  ...
  // 3. Map start and end positions to grid coordinates
  const startX = lngToX(startLoc.lng);
  const startY = latToY(startLoc.lat);
  const endX = lngToX(endLoc.lng);
  const endY = latToY(endLoc.lat);

  // Ensure start and end are not blocked (otherwise force them to be unblocked for pathing)
  ...
  if (!path) {
    // If blocked completely, return a direct straight-line path as fallback
    return [
  ...
  // 5. Convert grid coordinates back to LatLng coordinates
  const latLngPath = path.map(node => ({
    lat: yToLat(node.y),
    lng: xToLng(node.x)
  }));

  // Ensure exact start and end are set as the terminal points of the path
...
function runAStar(grid, startX, startY, endX, endY) {
  ...
  while (openList.length > 0) {
    ...
    for (const [dx, dy] of dirs) {
      ...
      if (grid[ny][nx] === 1) continue; // Obstacle/Danger Zone
